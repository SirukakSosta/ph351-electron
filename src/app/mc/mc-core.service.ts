...
export class McCoreService {
  ...
  start(gridSizes: number[], J: number, B: number, K: number, ITERATIONS: number, T0: number, T_MAX: number, T_STEP: number, spinChangesPerIteration: number) {
    ...
    for (let gridIndex = 0; gridIndex < gridSizes.length; gridIndex++) {
      ...
      let workerEvent$ = fromEvent<MessageEvent>(worker, "message").pipe(
        ...
        takeUntil(this.newExp$.pipe(tap(e => {
          worker.terminate()
        })))
      );

      const workerInput: McWorkerInput = { GRID_SIZE, ITERATIONS, T0, T_MAX, T_STEP, B, J, K, spinChangesPerIteration }
