...
export class McWrapperComponent implements OnInit {
  ...
  progress$ = this.service.calculationResults$$.pipe(
    ...
    map(results => {
      ...
      const stepsTilNow = results.map(e => e.tempratures.length).reduce((a, b) => a + b);
      const progress = 100 * (totalSteps - (totalSteps - stepsTilNow)) / totalSteps;
  ...
  start() {
    ...
    this.service.start(this.selectedGridSizes, this.J, this.B, this.K, this.ITERATIONS, this.T0, this.T_MAX, this.T_STEP, this.spinChangesPerIteration);
