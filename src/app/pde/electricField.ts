...
export class ElectricField {
  ...
  private calculateX_Vector() {
    ...
    for (let i = 0; i < size - 1; i++) {
      ...
      for (let j = 0; j < size - 1; j++) {
        this.xVector[i][j] =
          -(this.voltageMatrix[i + 1][j] - this.voltageMatrix[i][j]) / this.h; // eq 24
      }
    }
    ...
    for (let i = 0; i < size - 1; i++) {
      for (let j = 0; j < size - 1; j++) {
        this.xVector[i][j] =
          0.5 * (this.xVector[i][j + 1] + this.xVector[i][j]); // eq 26
      }
    }
  }
  ...
  private calculateY_Vector() {
    ...
    for (let i = 0; i < size - 1; i++) {
      ...
      for (let j = 0; j < size - 1; j++) {
        this.yVector[i][j] =
          -(this.voltageMatrix[i][j + 1] - this.voltageMatrix[i][j]) / this.h; // eq 25
      }
    }
    ...
    for (let i = 0; i < size - 1; i++) {
      for (let j = 0; j < size - 1; j++) {
        this.yVector[i][j] =
          0.5 * (this.yVector[i + 1][j] + this.yVector[i][j]); // eq 27
      }
    }
  }
}
