...
import { acceleration, displacementNextDt, kineticEnergy, potentialEnergy, velocityNextDt } from "../methods";
...
export class MdWrapperComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const displacementInitialMatrix = createArrayWithRandomNumbers(particleCount, dxStart, dxEnd, true); // initial displacement values for each particle
    const velocityInitialMatrix = createArrayWithRandomNumbers(particleCount, -0.1, 0.1, true); // initial velocity values for each particle
    ...
    const dtStartAcceleration = particleArray.map((e, i) => {
      ...
      const particleDisplacement = {
        previousParticle: displacementInitialMatrix[i - 1] || displacementInitialMatrix[particleCount - 1],
        currentParticle: displacementInitialMatrix[i],
        nextParticle: displacementInitialMatrix[i + 1] || displacementInitialMatrix[0]
      }
    ...
    const dtStartKineticEnergy = particleArray.map((e, i) => kineticEnergy(massMatrix[i], velocityInitialMatrix[i]));
    ...
    for (let t = dtStart; t < dtEnd; t += dt) {
      ...
      const _displacementNextDt = particleArray.map((e, i) => {
        ...
        const displacementNextDtForParticle_i = displacementNextDt(displacementTimeline.get(t)[i], velocityTimeline.get(t)[i], accelerationTimeline.get(t)[i], dt);
      ...
      const _accelerationNextDt = particleArray.map((e, i) => {
        ...
        const particleDisplacement = {
          previousParticle: displacementTimelineCurrentDt[i - 1] || displacementTimelineCurrentDt[particleCount - 1],
          currentParticle: displacementTimelineCurrentDt[i],
          nextParticle: displacementTimelineCurrentDt[i + 1] || displacementTimelineCurrentDt[0]
        }
        const accelerationNextDtForParticle_i = acceleration(particleDisplacement, massMatrix[i], constants);
      ...
      const _velocityNextDt = particleArray.map((e, i) => {
        ...
        const velocityNextDtForParticle_i = velocityNextDt(velocityTimeline.get(t)[i], accelerationTimeline.get(t)[i], accelerationTimeline.get(dtNext)[i], dt);
      ...
      const _kineticEnergyNextDt = particleArray.map((e, i) => {
        ...
        const kineticEnergyNextDtForParticle_i = kineticEnergy(massMatrix[i], velocityTimeline.get(dtNext)[i]);
      ...
      const _potentialEnergyNextDt = particleArray.map((e, i) => {
        ...
        const potentialEnergyNextDtForParticle_i = potentialEnergy({
          ...
          nextParticle: displacementTimeline.get(dtNext)[i + 1] || displacementTimeline.get(dtNext)[0]
        }, constants);
    ...
    console.log('displacementTimeline', Array.from(displacementTimeline.values()))
    console.log('velocityTimeline', Array.from(velocityTimeline.values()))
    console.log('accelerationTimeline', Array.from(accelerationTimeline.values()))
    console.log('kineticEnergyTimeline', Array.from(kineticEnergyTimeline.values()))
    console.log('potentialEnergyTimeline', Array.from(potentialEnergyTimeline.values()))
